<aside class="flex  flex-col text-white bg-[#495E57] transition-all duration-300 ease-in-out min-h-screen"
    x-data="{ 'active': location.pathname }" :class="isSidebarExpanded ? 'w-64 min-w-[15%]' : 'w-20'">
    <div class="sticky top-0">
        ...
        <nav class="p-4 space-y-2 font-medium">
            <a href="{{ route('dashboard') }}"
                ...
                :class="active.startsWith('/dashboard') ? 'text-[#495E57] bg-white' :
                    'text-white bg-[#495E57] hover:bg-white hover:bg-opacity-75'">
                <img class=" max-h-full aspect-square" :class="isSidebarExpanded ? 'p-1' : ''"
                    :src="'{{ asset('img/nav.dashboard') }}' + (active.startsWith('/dashboard') ? '' : '.white') + '.svg'" />
            ...
            <a href="{{ route('planting-planning.index') }}"
                ...
                :class="active.startsWith('/planting-planning') ? 'text-[#495E57] bg-white' :
                    'text-white bg-[#495E57] hover:bg-white hover:bg-opacity-75'">
                <img class=" max-h-full aspect-square" :class="isSidebarExpanded ? 'p-1' : ''"
                    :src="'{{ asset('img/nav.schedules') }}' + (active.startsWith('/planting-planning') ? '' : '.white') +
                    '.svg'" />
                <span class="ml-2 duration-300 ease-in-out text-lg text-inherit"
            ...
            <a href="{{ route('supplies.index') }}"
                class="flex items-center h-10 px-3  rounded-lg transition-colors duration-150 ease-in-out focus:outline-none focus:shadow-outline"
                :class="active.startsWith('/supplies') ? 'text-[#495E57] bg-white' :
                    'text-white bg-[#495E57] hover:bg-white hover:bg-opacity-75'">
                <img class=" max-h-full aspect-square" :class="isSidebarExpanded ? 'p-1' : ''"
                    :src="'{{ asset('img/nav.supplies') }}' + (active.startsWith('/supplies') ? '' : '.white') +
                    '.svg'" />
                <span class="ml-2 duration-300 ease-in-out text-lg text-inherit"
